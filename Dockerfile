...
# [EKLENDİ] Ağ sorunlarını ve zaman aşımlarını önlemek için Ubuntu Türkiye sunucularına yönlendirme
RUN sed -i 's/archive.ubuntu.com/tr.archive.ubuntu.com/g' /etc/apt/sources.list.d/ubuntu.sources || true
...
# [EKLENDİ] Asıl hatanın patladığı aşama burasıydı. Tekrar ağ sorunu yaşanmaması için sunucuyu değiştiriyoruz.
RUN sed -i 's/archive.ubuntu.com/tr.archive.ubuntu.com/g' /etc/apt/sources.list.d/ubuntu.sources || true
